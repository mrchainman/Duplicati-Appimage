...
APP=duplicati-beta-bin
BIN="duplicati" #CHANGE THIS IF THE NAME OF THE BINARY IS DIFFERENT FROM "$APP" (for example, the binary of "obs-studio" is "obs")
DEPENDENCES="lib32-icu icu lib32-ice lib-ice lib32-libice libice libsm lib32-libsm" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
if test -f "$APP".AppDir/*.desktop; then
...
elif test -f archlinux/.junest/opt/duplicati/"$BIN"; then
	...
	Comment=
	Exec=/opt/duplicati/$BIN --webservice-port=8200 --auto-update=false
	Icon=tux
	Categories=Utility;
	Terminal=true
	StartupNotify=true
...
if ! echo "$EXEC" | grep -q "/usr/bin"; then EXEC="$EXEC"; fi
